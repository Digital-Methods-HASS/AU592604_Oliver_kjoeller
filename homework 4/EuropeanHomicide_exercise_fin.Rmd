...
ggplot(data=Western_Europe, mapping=aes(x=Year, y=homicides_per_100k)) + 
  geom_jitter(mapping = aes(colour=Entity)) + 
  labs(x="The year", y="total homicides per 100.000 people")
```
...
```{r eval = FALSE}
kings_data <- read.csv("my-kings.csv")
kings_data$total <- (kings_data$End-kings_data$Start)
kings_data$Mid <- (kings_data$End - (kings_data$End-kings_data$Start)/2)
kings_data %>%
  ggplot(aes(x = Mid, y= total)) +
  geom_smooth() +
  labs(title = "reign through time", x = "Year", Y= "Duration")
```

## Final tasks:

1)  Plot: In the facetted plot above, move the legend from the current position on the side to below the facets, and label it "Country" instead of "Entity". y

2)  Rmarkdown:

-   edit the author of the document, and convert 'Final Tasks' into heading #2 (like the other headings) y
-   add a `floating table of contents` to your Rmarkdown document, y
-   provide informative `chunk-names` and edit flags in your R chunks, and y
...
3)  Question: In \<250 words articulate your answer on the basis of the data visualisations to the following question: are we more civilized today? #Even after looking at multiple countries it seems to be the case that homicides are indeed declining. Im not really sure if this qualifies as a necessarily more "civiilized" country but if we assume our data to be accurate its hard to deny that it seems to be the case. Homicides are on the decline at least.
